...
// Режим SPI: без прерываний, старший бит первый, режим мастера, CPOL->0, CPHA->0, Clk/4
#define SPI_MODE					0x50

#define LCD_PORT                   TO_PORT(PORT_LCD)
#define LCD_DDR                    TO_DDR(PORT_LCD)

// Распиновка порта
#define LCD_DC_PIN                 PORT_B1_O_DC_LCD
#define LCD_CE_PIN                 PORT_B2_O_CE_LCD
#define SPI_MOSI_PIN               PORT_B3_O_SD_LCD   // SDIN дисплея обязательно подключаем к MOSI аппаратного SPI
//#define LCD_RST_PIN                PB1
#define SPI_CLK_PIN                PORT_B5_O_CLK_LCD   // SCLK дисплея обязательно подключаем к SCK аппаратного SPI
...
byte LcdChr        ( LcdFontSize size, byte ch, LcdPixelMode mode );   // Вывод символа в текущей позиции
byte LcdStr        ( LcdFontSize size, const char *s, LcdPixelMode mode );   // Вывод строки сохраненной в RAM
byte LcdFStr       ( LcdFontSize size, const byte *dataPtr, LcdPixelMode mode );   // Вывод строки сохраненной в Flash ROM
