...
*/
 ...
 //   SPCR = SPI_MODE;
    ...
    LcdSend( 0x08 | 1 , LCD_CMD ); // of HV-gen voltage multiplication
    LcdSend( 0x80 | 0x48, LCD_CMD ); // Установка контрастности (LCD Vop)-c8
    LcdSend( 0x06, LCD_CMD ); // Установка температурного коэффициента (Temp coefficent)
    LcdSend( 0x14, LCD_CMD ); // Настройка питания (LCD bias mode 1:48)-13
...
//    LcdSend( 8+4+1, LCD_CMD ); // Нормальный режим (LCD in normal mode)
    LcdOff();
